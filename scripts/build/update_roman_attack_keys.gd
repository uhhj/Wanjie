extends SceneTree
## Export attack key tracks only; preserve rig and other animations.
func _initialize() -> void:
	var data: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://resources/human_medium_rig_v1.json"))
	var entry: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://resources/roman_guard_animations_v1.json")).attack_01
	var library: AnimationLibrary = load("res://resources/roman_guard_animations_v1.tres")
	var walk: Animation = library.get_animation("attack_01").duplicate(true)
	...
	for track in range(walk.get_track_count()):
		...
		if not values.has(path):
			continue
		while walk.track_get_key_count(track)>0:
			walk.track_remove_key(track,0)
		for i in range(entry.times.size()):
			walk.track_insert_key(track,float(entry.times[i]),values[path][i])
	walk.length = float(entry.length)
	for track in range(walk.get_track_count()):
		if walk.track_get_type(track)==Animation.TYPE_METHOD:
			walk.track_set_key_time(track,0,float(entry.method_events[0].time))
	var output = "res://reports/roman_heavy_slash/attack_animation.tres"
	assert(ResourceSaver.save(walk,output)==OK)
	print("ATTACK KEYS EXPORTED; other animations retained")
	quit()
