...
func run() -> void:
	...
	for i in 271:
		...
		hand_error=maxf(hand_error,Vector2(414.0,-625.0).distance_to(hand))
	...
	FileAccess.open("res://reports/minotaur_breaker/"+prefix+"skill_v5_test.json",FileAccess.WRITE).store_string(JSON.stringify(result,"  ")+"\n")
