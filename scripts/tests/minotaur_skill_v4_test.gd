...
func run() -> void:
	...
	for i in 271:
		...
		hand_error=maxf(hand_error,Vector2(274.0,-535.0).distance_to(hand))
	...
	if blade_forward_min<150.0:errors.append("Axe blade enters the body plane: "+String.num(blade_forward_min,1))
	if blade_drop_min<300.0:errors.append("Axe blade rides too high")
	...
	var result:={"status":"PASS" if errors.is_empty() else "FAIL","scene_sha256":FileAccess.get_sha256(scene),"far_hand_tracking_error_source_px":hand_error,"min_charge_lean_deg":lean_min,"blade_min_forward_offset_source_px":blade_forward_min,"blade_min_drop_source_px":blade_drop_min,"root_distance_source_px":2880.0,"skill_hit_count":counts[1],"errors":errors}
	var prefix:="formal_" if "--formal" in OS.get_cmdline_user_args() else ""
	FileAccess.open("res://reports/minotaur_breaker/"+prefix+"skill_v4_test.json",FileAccess.WRITE).store_string(JSON.stringify(result,"  ")+"\n")
