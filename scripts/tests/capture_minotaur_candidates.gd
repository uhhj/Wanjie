...
const FOLDER := "res://reports/minotaur_breaker/animations"
...
func run() -> void:
	...
	for height in heights:
		...
		view.size = Vector2i(800,560)
		view.render_target_update_mode = SubViewport.UPDATE_ALWAYS
		root.add_child(view)
		var bg := ColorRect.new()
		bg.size = Vector2(800,560)
		bg.color = Color(.85,.86,.87)
		view.add_child(bg)
		var ground := Line2D.new()
		ground.points = PackedVector2Array([Vector2(20,475),Vector2(780,475)])
		ground.width = 1
	...
	for name in names:
		...
		for i in 16:
			var t := length*i/float(16 if name in ["idle","walk"] else 15)
			for unit in units:
				unit.play_animation(name)
				unit.animation_player.seek(t,true)
				unit.position = Vector2(300+unit.root_distance*unit.scale.x,475)
			await process_frame
	...
	var prior_path:="res://reports/minotaur_breaker/capture_manifest.json"
	...
	var f := FileAccess.open("res://reports/minotaur_breaker/capture_manifest.json",FileAccess.WRITE)
