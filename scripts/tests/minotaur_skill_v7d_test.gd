...
func run() -> void:
	...
	for i in 271:
		...
		hand_error=maxf(hand_error,Vector2(372.0,-573.0).distance_to(hand))
	...
	FileAccess.open("res://reports/minotaur_breaker/"+prefix+"skill_v7d_test.json",FileAccess.WRITE).store_string(JSON.stringify(result,"  ")+"\n")
