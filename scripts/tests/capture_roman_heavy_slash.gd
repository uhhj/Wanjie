...
func capture() -> void:
	...
	for name in names:
		...
		for frame in range(frame_count):
			...
			for index in range(heights.size()):
				var folder = walk_folder if walk_only else "reports/roman_heavy_slash"
	...
	var file = FileAccess.open("res://"+walk_folder+"/capture_manifest.json" if walk_only else "res://reports/roman_heavy_slash/capture_manifest.json",FileAccess.WRITE)
