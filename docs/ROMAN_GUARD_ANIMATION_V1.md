...
## Walk 参考步态 V1（历史，后续由小腿联动 V2 修正）

参照用户提供的抬跟/后腿收回姿态条，只修订 Walk 的既有关键帧。1 秒循环、49 个插值关键姿态、每周期前进 387 源像素及两腿半周期错相保持不变。后足接地后放平，支撑后段抬跟，以前脚掌蹬地；离地后先收回小腿，再经过 passing 并向前落脚。足部世界角从约 -12° 到 +28°。屈膝峰值提前到收腿阶段，未增加原近膝关节形变上限。

骨盆保持 0–7 源像素小幅起伏，时机改为接地后承重下沉、通过后抬升；极小前后重心、上身反向补偿、持剑臂摆动和稳定持盾保持上一版设置。near_knee 节点、pivot、蒙皮和所有美术文件保持原样。

脚底检测改为测量实际滚动时接地的鞋底材料点：从冻结鞋底轮廓校准五个点，在每个接触区间分别检查世界位置，并独立检查支撑面高度。不能再把正在抬起的脚跟当作滑步测量点。256px 最大误差约 0.089px，192px 约 0.067px。

`reports/walk_reference_v1/` 同时保存前进视角与 Camera2D 跟随视角；均为原生 GPU 渲染，每份 GIF 为 16 帧/1 秒。跟随视角的地面刻度向后移动，角色仍使用前进位移。全部非 Walk 动画及 Rig/部件冻结验证通过；本次未重测旧桌面 FPS。动作自然程度留待用户看新版确认，旧“将就了吧”的保留意见不视为本次认可。


## Walk 小腿/脚掌联动 V2（当前）

用户指出脚掌改变角度而小腿未同步。V1 的近侧脚在蹬地时局部踝角达到 42.19°，小腿轴仍近乎竖直，且屈膝在离地后才开始。本次把屈膝提前到周期 0.25–0.50 的承重后段，连续进入离地收腿，再平滑伸展。近侧脚步轨迹向后调整 30 源像素，位置变化由既有大腿/小腿关键帧与髋部位置键求解；未移动 Rig rest pivot。

足部世界抬跟峰值由 28° 收到 18°；近侧蹬地小腿轴从竖直前方约 1.5° 改为后倾约 12.6°，相对踝角降为 18.10°。近膝原有形变上限不增加。上身、骨盆、双臂、头、盾和 knee_near 的键值与上一版完全一致；其余四个动画、部件与 Rig 绑定也完全保留。

支撑点真实 Godot 世界漂移约 0.038px（256px），192px 约 0.029px。新 16 帧渲染、前后对照与键值记录在 `reports/walk_chain_v2/`。本次只确认技术与战斗尺寸连接检查完成，未把用户对前版的否定记录成新版认可。
