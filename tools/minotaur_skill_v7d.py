"""Skill V5: extended forward carry. The axe hand stays nearly straight the
whole clip and the axe angle follows the forearm, so neither wrist nor forearm
can fold past anatomical range. The free arm just swings."""
import math
import numpy as np
...
def build_skill(m):
    ...
    for t in times:
        p=m.base();context=f'skill_v7d:{t:.3f}'
        ...
        # Extended forward carry: the hand pushes out ahead of the chest and the
        # elbow solves forward, keeping the chain at 65-95% extension. The axe
        # orientation is derived from the solved forearm angle, so the wrist
        # stays at its natural rest offset by construction.
        sway=4*math.sin(phase*2*math.pi) if running else 0
        grip_x=float(m.lerp_keys(t,[(0,[778]),(.30,[808]),(.70,[845]),(1.10,[858]),(3.48,[858]),(3.95,[808]),(4.2,[778])])[0])
        grip_y=float(m.lerp_keys(t,[(0,[850]),(.30,[892]),(.70,[905]),(1.10,[912]),(3.48,[912]),(3.95,[892]),(4.2,[850])])[0])
        q=m.base();q['pelvis_offset']=p['pelvis_offset'];q['rotations']['torso']=p['rotations']['torso']
        # Pin the far shoulder bone at the art deltoid (same mechanism as the
        # attack lift): without this the 56-degree lean drags the shoulder root
        # down to the belt and the forearm mesh reads severed.
        p['bone_offsets']={'arm_far_upper':[3.*envelope,-90.*envelope]}
        q['bone_offsets']={'arm_far_upper':[3.*envelope,-90.*envelope]}
        m.solve(q,'arm_far_upper','arm_far_fore','hand_far',np.array([grip_x,grip_y]),0.0,-1,context)
        phi=math.degrees(m.fk(q,'arm_far_fore')[1])
        hand_orient=max(-10.,min(phi-39.,14.))+sway*float(envelope>0)
        q['rotations']['hand_far']=hand_orient-math.degrees(m.fk(q,m.B['hand_far']['parent'])[1])
        approach=float(m.smooth(float(np.clip((t-.05)/.30,0,1))))
        ...
        for bone in ['arm_far_upper','arm_far_fore','hand_far']:
            ...
            p['rotations'][bone]=float(v*approach*(1-release))
