...
def main():
    ...
    for height in [256,192]:
        ...
        gif=folder+f'walk_polish_{height}.gif'
        ...
        sheet=folder+f'walk_polish_{height}_review.png';out.save(ROOT/sheet)
        outputs[str(height)]={'gif':gif,'frames':16,'duration_ms':1000,'sheet':sheet,'gif_sha256':sha(gif),'sheet_sha256':sha(sheet)}
    metrics={'status':'TECHNICAL_PASS_VISUAL_REVIEW_REQUIRED','other_animations_unchanged':True,'rig_outside_walk_resource_unchanged':True,'frozen_files_verified':len(snapshot)-len(allowed),'near_knee_unchanged':True,'gait_structure_unchanged':True,'pelvis_vertical_range_old':[0,4],'pelvis_vertical_range_new':[0,7],'pelvis_fore_aft_source_px':[-1.5,1.5],'stride_source_px_old':360,'stride_source_px_new':387,'stride_increase_percent':7.5,'swing_lift_source_px_old':30,'swing_lift_source_px_new':34,'sword_upper_arm_added_degrees':0.8,'shield_forearm_inertia_degrees':0.3,'support_drift_at_256px':native['runtime_support_drift_at_256px'],'support_drift_at_192px':native['runtime_support_drift_at_256px']*.75,'outputs':outputs,'animation_source_sha256':sha('resources/roman_guard_animations_v1.json')}
    write(folder+'walk_polish_metrics.json',metrics)
