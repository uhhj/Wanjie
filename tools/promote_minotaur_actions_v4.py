"""Attack V4 + charge V3 promotion, preserving the four other clips and all rig assets."""
...
def main():
    capture=read(R/'actions_v4_capture_manifest.json');review=read(R/'actions_v4_visual_review.json')
    ...
    for name in ['attack_v4_test','skill_v3_test','candidate_ik_reach','candidate_format_validation']:assert read(R/(name+'.json'))['status']=='PASS'
    ...
    for name in ['idle','walk','hit','death']:assert previous[name]==current[name],name
    ...
    report={'status':'PROMOTED_PENDING_FORMAL_TEST','base_commit':BASE,'changed_clips':['attack_01','skill_01'],'unchanged_clips':['idle','walk','hit','death'],'unchanged_parts':21,'unchanged_rest_rig':True,'impact_fx':'Deterministic progress-track node, no new art files','formal_scene_sha256':sha(target)}
    (R/'actions_v4_promotion.json').write_text(json.dumps(report,indent=2)+'\n')
