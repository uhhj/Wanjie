...
def main():
    capture=read(R/'actions_v4_capture_manifest.json');review=read(R/'skill_v4_visual_review.json')
    ...
    for name in ['skill_v4_test','attack_v4_test','candidate_ik_reach','candidate_format_validation']:assert read(R/(name+'.json'))['status']=='PASS'
    ...
    report={'status':'PROMOTED_PENDING_FORMAL_TEST','base_commit':BASE,'changed_clips':['skill_01'],'unchanged_clips':['idle','walk','attack_01','hit','death'],'unchanged_parts':21,'unchanged_rest_rig':True,'carry':'Front-low axe carry; hand and blade outside the body silhouette','formal_scene_sha256':sha(target)}
    (R/'skill_v4_promotion.json').write_text(json.dumps(report,indent=2)+'\n')
