"""Attack-only promotion, preserving the five other clips and all rig assets."""
...
def main():
    capture=read(R/'attack_v3_capture_manifest.json');review=read(R/'attack_v3_visual_review.json')
    ...
    assert len(capture['frames'])==64 and len({(f['animation'],f['height'],f['frame']) for f in capture['frames']})==64
    ...
    for name in ['attack_v3_test','candidate_ik_reach','candidate_format_validation']:assert read(R/(name+'.json'))['status']=='PASS'
    ...
    for name in ['idle','walk','skill_01','hit','death']:assert previous[name]==current[name],name
    ...
    report={'status':'PROMOTED_PENDING_FORMAL_TEST','base_commit':BASE,'changed_clips':['attack_01'],'unchanged_clips':['idle','walk','skill_01','hit','death'],'unchanged_parts':21,'unchanged_rest_rig':True,'formal_scene_sha256':sha(target)}
    (R/'attack_v3_promotion.json').write_text(json.dumps(report,indent=2)+'\n')
