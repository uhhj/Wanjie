...
def main():
    capture=read(R/'actions_v4_capture_manifest.json');review=read(R/'skill_v5_visual_review.json')
    ...
    for name in ['skill_v5_test','attack_v4_test','candidate_ik_reach','candidate_format_validation']:assert read(R/(name+'.json'))['status']=='PASS'
    ...
    report={'status':'PROMOTED_PENDING_FORMAL_TEST','base_commit':BASE,'changed_clips':['skill_01'],'unchanged_clips':['idle','walk','attack_01','hit','death'],'unchanged_parts':21,'unchanged_rest_rig':True,'carry':'Extended forward upright carry; wrist angle derived from the forearm','formal_scene_sha256':sha(target)}
    (R/'skill_v5_promotion.json').write_text(json.dumps(report,indent=2)+'\n')
