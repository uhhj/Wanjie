...
def main():
    ...
    lines+=['','**当前 Walk 已获用户明确确认：“确定为这版，将不合格的删除”。其他动作维持各自既有审批；Attack 仍保留此前的原型质量意见。**']
    if 'walk_chain_review' in g:
        lines+=['','当前 Walk 修正了小腿与脚掌联动：屈膝提前到蹬地阶段，收小踝部单独折动。用户已确认采用此版。[16帧跟随视角](walk_chain_v2/walk_reference_follow_256.gif) / [本次报告](WALK_CHAIN_COUPLING_V2.md)。']
