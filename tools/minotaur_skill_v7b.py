...
def build_skill(m):
    ...
    for t in times:
        p=m.base();context=f'skill_v7b:{t:.3f}'
        ...
        grip_x=float(m.lerp_keys(t,[(0,[778]),(.30,[815]),(.70,[860]),(1.10,[880]),(3.48,[880]),(3.95,[820]),(4.2,[778])])[0])
        grip_y=float(m.lerp_keys(t,[(0,[850]),(.30,[885]),(.70,[878]),(1.10,[870]),(3.48,[870]),(3.95,[878]),(4.2,[850])])[0])
        ...
        q['bone_offsets']={'arm_far_upper':[3.*envelope,-90.*envelope],'axe_socket':[0.,-455.*envelope]}
        m.solve(q,'arm_far_upper','arm_far_fore','hand_far',np.array([grip_x,grip_y]),0.0,-1,context)
        phi=math.degrees(m.fk(q,'arm_far_fore')[1])
        hand_orient=phi+80.*envelope+sway*float(envelope>0)
